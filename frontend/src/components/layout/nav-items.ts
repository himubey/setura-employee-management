...
/**
 * Navigation is data, not markup — the sidebar and the mobile nav render the
 * same list, so a new section is added in exactly one place.
 *
 * `icon` is an inline SVG path string rather than an icon-library component,
 * which keeps the dependency count at zero for navigation chrome.
 */
export type NavItem = {
  /** `Route` (not `string`) so a typo is a build error, via typedRoutes. */
  href: Route;
  label: string;
...
export const NAV_ITEMS: readonly NavItem[] = [
  ...
  },
  {
    href: "/employees",
    label: "Employees",
    // Must match the page's own guard (`requirePermission("employee:read_all")`),
    // or a plain employee sees the link and hits the error boundary.
